...
class EstudianteGrupoController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->has('estudiante') && is_numeric($request->estudiante_id)) {
            $query->where('estudiante', '=', $request->estudiante);
        }
        $estudiantesGrupos = $query->with('estudiante', 'grupo')
            ->orderBy('id', 'desc')
            ->simplePaginate(10);
        $destudiantesGrupos = $query->orderBy('id', 'desc')->simplePaginate(10);
